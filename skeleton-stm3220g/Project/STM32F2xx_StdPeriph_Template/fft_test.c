...
void fft_test(FFT_signals_t *signals, float *expected_real_output, float* expected_imag_ouput, float* expected_magnitude, bool* test_results, int n) {
	...
	test_results[1] = check(buffer_compare(signals->imag_input, expected_imag_ouput, n), "test_imag");
...
bool test_1() {
		...
		float *out_r, *out_i, *mag;
		
		out_r = calloc(4, sizeof(float));
		out_i = calloc(4, sizeof(float));
		mag = calloc(4, sizeof(float));
		FFT_signals_t *test_signals = calloc(1, sizeof(FFT_signals_t));
		
		if (!out_r || !out_i || !mag || !test_signals) {
...
bool test_2() {
	...
	out_r = calloc(FFT_LENGTH, sizeof(float));
	out_i = calloc(FFT_LENGTH, sizeof(float));
	mag = calloc(FFT_LENGTH, sizeof(float));
